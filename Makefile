...
config:
	@curl -s -X POST http://localhost:8001/services/ -d 'name=private' -d url=http://httpbin.org/anything
	@curl -s -X POST http://localhost:8001/services/private/routes -d 'paths[]=/private' 
	@curl -i -X POST http://localhost:8001/services/private/plugins -F "name=${NAME}"
